...
sgs.ai_use_priority.WuSixSwords = 2.65
sgs.ai_use_priority.TripleAngleBlade = 2.675
sgs.weapon_range.WuSixSwords = 2
sgs.weapon_range.TripleAngleBlade = 3

sgs.ai_skill_use["@@wusix"] = function(self, prompt)
	...
	if fifth_index then
		return ("@WuSixCard=.->%s+%s+%s+%s+%s"):format(first, second, third, forth, fifth)
	elseif forth_index then
		return ("@WuSixCard=.->%s+%s+%s+%s"):format(first, second, third, forth)
	elseif third_index then
		return ("@WuSixCard=.->%s+%s+%s"):format(first, second, third)
	elseif second_index then
		return ("@WuSixCard=.->%s+%s"):format(first, second)
	elseif first_index then
		return ("@WuSixCard=.->%s"):format(first)
	end

	if not first_index then return "." end
end

sgs.ai_card_intention.WuSixCard = -50

sgs.ai_skill_invoke.triple_angle_blade = function(self, data)
	local damage = data:toDamage()
	local enemynum = 0
...
sgs.ai_skill_playerchosen.triple_angle_blade = function(self, targets)
	local tos = {}
...
sgs.ai_playerchosen_intention.triple_angle_blade = 80

function SmartAI:useCardAttackExhausted(card, use)
...
sgs.ai_use_value.AttackExhausted = 3
sgs.ai_use_priority.AttackExhausted = 2.75
sgs.ai_card_intention.AttackExhausted = -30

function SmartAI:useCardKnowAll(card, use)    
	use.card = card
...
sgs.ai_use_value.KnowAll = 5.4
sgs.ai_use_priority.KnowAll = 2.8

function SmartAI:useCardBefriendDistant(card, use)
	use.card = card
	local players = self.room:getOtherPlayers(self.player)
	players = sgs.QList2Table(players)
	local target
...
sgs.ai_keep_value.BefriendDistant = 3.6
sgs.ai_use_value.BefriendDistant = 10
sgs.ai_use_priority.BefriendDistant = 6

-- AI for general

sgs.ai_skill_invoke.xiaoguo = function(self, data)
...
duoshi_skill.getTurnUseCard=function(self,inclusive)
	...
	for _,card in ipairs(cards)  do
		if card:isRed() then
			...
			if self:getUseValue(card) > sgs.ai_use_value.AttackExhausted and card:inherits("TrickCard") then
				local dummy_use = {isDummy = true}
	...
	if red_card then
		...
		local card_str = ("attack_exhausted:duoshi[%s:%s]=%d"):format(suit, number, card_id)
...
sgs.ai_playerchosen_intention.lirang = -50
...
sgs.ai_skill_use["@@huoshui"] = function(self, prompt)
	local first_index, second_index, third_index, forth_index, fifth_index
	self:sort(self.enemies, "defense")
	if #self.enemies < 1 then return "." end
	local sum
	if self.player:getMark("huoshui") >= #self.enemies then
		sum = #self.enemies
	else
		sum = self.player:getMark("huoshui")
	end
	for i=1, sum do
		...
		elseif not second_index  then
			second_index = i
		elseif not third_index  then
			third_index = i
		elseif not forth_index then
			forth_index = i
		else
			fifth_index = i
		end
	...
	local first, second, third, forth, fifth
	if first_index then first = self.enemies[first_index]:objectName() end
	if second_index then second = self.enemies[second_index]:objectName() end
	if third_index then third = self.enemies[third_index]:objectName() end
	if forth_index then forth = self.enemies[forth_index]:objectName() end
	if fifth_index then fifth = self.enemies[fifth_index]:objectName() end
	if fifth_index then
		return ("@HuoshuiCard=.->%s+%s+%s+%s+%s"):format(first, second, third, forth, fifth)
	elseif forth_index then
		return ("@HuoshuiCard=.->%s+%s+%s+%s"):format(first, second, third, forth)
	elseif third_index then
		return ("@HuoshuiCard=.->%s+%s+%s"):format(first, second, third)
	elseif second_index then
		return ("@HuoshuiCard=.->%s+%s"):format(first, second)
	elseif first_index then
		return ("@HuoshuiCard=.->%s"):format(first)
	end

	if not first_index then return "." end
end

sgs.ai_skill_choice.huoshui = function(self, choices)
	return "yes"
end
...
qingcheng_skill.getTurnUseCard=function(self)
	if not self.player:isNude() then return sgs.Card_Parse("@QingchengCard=.") end
end

sgs.ai_skill_use_func.QingchengCard=function(card,use,self)
	local cards = self.player:getCards("h3")
	cards=sgs.QList2Table(cards)
	local equipcard
	for _,acard in ipairs(cards)  do
	...
	if equipcard then
		...
		for _, enemy in ipairs(self.enemies) do
			local enemyseat = (enemy:getSeat() - self.player:getSeat()) % (global_room:alivePlayerCount())
			if self:objectiveLevel(enemy) > 3 and enemyseat > 1 then
				use.card = sgs.Card_Parse("@QingchengCard=" .. equipcard:getId())
				if use.to then
					use.to:append(enemy)
				end
				break
...
sgs.ai_use_value.QingchengCard = 2.5

sgs.ai_card_intention.QingchengCard = 80
